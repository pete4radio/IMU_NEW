...
4. Invoke the "bmm150_get_interrupt_status" API to get the interrupt status value 

   Interrupt status value is taken and a bitwise AND operation is performed with predefined interrupt status macros (listed below) to find the interrupt status which is either set or reset.
...
``` c
 ...
 if (int_status & BMM150_INT_ASSERTED_DRDY) {
...
int8_t high_threshold_interrupt_handling(struct bmm150_dev *dev)
{
	...
	if (rslt == BMM150_OK) {
		if (int_status & BMM150_INT_THRESHOLD_X_HIGH) {
			printf("\n HIGH THRESHOLD INTERRUPT ON X AXIS ");
		}
		if (int_status & BMM150_INT_THRESHOLD_Y_HIGH) {
			printf("\n HIGH THRESHOLD INTERRUPT ON Y AXIS ");
		}
		if(int_status & BMM150_INT_THRESHOLD_Z_HIGH) {
...
int8_t drdy_overflow_int_handling(struct bmm150_dev *dev)
{
	...
	if (rslt == BMM150_OK) {
		...
		if ((int_status & BMM150_INT_ASSERTED_DRDY) ||
			(int_status & BMM150_INT_DATA_OVERFLOW)) {
